...
class RecordVideoWrapper(EnvironmentWrapper):
    def __init__(self, 
        ...
        Args:
            ...
            record_every (int, optional): Record every `record_every` episodes. Defaults to 1.
            control_rate (int, optional): The control rate / action rate of the Environment to be wrapped. Defaults to 100.
            camera_id (str, optional): The Camera-ID used when recoding. Defaults to None.
            width (int, optional): Pixel-count in width. Defaults to 640.
            height (int, optional): Pixel-count in heigth. Defaults to 480.
            video_name (str, optional): Name of the video-file save. By default into `./videos/episode_X.mp4`. Defaults to None.
            cleanup_imgs (bool, optional): If enabled will automatically delete the generated images for recording the video. Defaults to False.
        """
    ...
    def step(self, action) -> dm_env.TimeStep:
        ...
        if record_episode and record_step:
            if self._camera_id:
                frame = self._environment.physics.render(camera_id=self._camera_id,width=self._width, height=self._height)
            else:
                frame = self._environment.physics.render(width=self._width, height=self._height)
    ...
    def _make_video(self):
        ...
        call(
            f"ffmpeg -r {self._fps} -i {prefix}/frame%03d.png -y {prefix}/{video_name} > /dev/null",
            stdout=DEVNULL,
            stderr=STDOUT,
            shell=True
        )
