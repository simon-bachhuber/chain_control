...
def _train(
    module,
    step_fn_missing_training_options,
    ...
    losses = loop.gogogo(
        training_options.n_gradient_steps, module, opt_state, minibatch_state
    )
...
def train_model(
    ...
    if training_options.eval_test_loss:
        if test_sample is None:
            raise Exception(
                "You have to provide the argument `test_sample` if `training_options.\
                    eval_test_loss` is `True`"
            )
