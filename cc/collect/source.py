...
def constant_after_transform_source(source: ObservationReferenceSource, after_T: float, new_ts: jnp.ndarray = None) -> ObservationReferenceSource:
    if new_ts is None:
        new_ts = source._ts 
    ...
    return ObservationReferenceSource(new_ts, to_numpy(source.get_references_for_optimisation()))
