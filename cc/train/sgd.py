...
def pbar_desc_(pbar, train_loss, test_loss, module):
    if test_loss:
        pbar.set_description(
            "Trainings-Loss: {:10.4f} | Test-Loss: {:10.4f} | \
                ParamsRegu: {:10.4f}".format(
                train_loss, test_loss, jnp.mean(flatten_module(module) ** 2)
            )
        )
    else:
        pbar.set_description(
            "Trainings-Loss: {:10.4f} | ParamsRegu: {:10.4f}".format(
                train_loss, jnp.mean(flatten_module(module) ** 2)
            )
        )
...
class SGD_Loop:
    ...
    def gogogo(
        self,
        steps: int,
        module: Optional[AbstractWrappedRHS] = None,
        opt_state=None,
        minibatch_state: Optional[MiniBatchState] = None,
    ):
        ...
        for i in pbar:
            self._module, self._opt_state, self._minibatch_state, value = self._step_fn(
                self._module, self._opt_state, self._minibatch_state
            )
            ...
            self._model_report.report(
                self._module, train_loss if test_loss is None else test_loss
            )
