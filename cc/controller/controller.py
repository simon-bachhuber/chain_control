...
from ..abstract import AbstractController, AbstractRHS
...
from ..types import NotAParameter, PossibleParameter, PyTree, TimeSeriesOfAct
...
def create_pi_controller(
    p_gain: float, i_gain: float, delta_t: float
...
class FeedforwardRHS(AbstractRHS):
    ...
    def __call__(
        self, state: FeedforwardCounterState, x: PyTree
    ...
    def init_state(
        self,
    ) -> Tuple["FeedforwardRHS", PossibleParameter[FeedforwardCounterState]]:
        # initialise counter
